...
statement:  blocks
            ...
            {
                symbol_type a = $1;
                if(a.type == PROC_TYPE) Trace("Call procedure");
            }
...
simple: IDENT DEC expr
        ...
        */
        | RESULT expr
        {
            ...
            if(func_ident_success()) {
                ...
                if(res == SUCCESS) {Trace("Result expression successful");}
                else {
                    insert_function_err();
                    yyerror(err_msg[res]);
                }
        ...
        */
        | RETURN
        {
            ...
            if(in_proc() == SUCCESS) {Trace("Return expression successful");}
            else {
                insert_function_err();
...
loop:   LOOP
        ...
        */
        | FOR IDENT ':' expr TD expr
        {
            ...
            create();
            if(a.is_const == false || b.is_const == false) yyerror(err_msg[MUST_BE_CONST]);
            else {
                if(a.val.type != INT_TYPE || b.val.type != INT_TYPE) yyerror(err_msg[MUST_BE_INT]);
                else {
                    id_type temp = {
                        .type = INT_TYPE,
                        .int_val = b.val.int_val
                    };
                    insert($2, temp, false);
                }  
            }
        ...
        | FOR DECREASING IDENT ':' expr TD expr
        {
            Trace("For Decreasing");
            expr_id a = $5;
            expr_id b = $7;
...
variable_dec:   VAR IDENT ':' variable_type
                {
                    ...
                    if(res == VAR_DECLARED) yyerror(err_msg[res]);
                    else Trace("Success Insertion Type Only");
                }
                | VAR IDENT DEC expr
                {
